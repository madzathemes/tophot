...
<article id="post-<?php the_ID(); ?>" class="<?php if ( is_sticky() and !is_single()){ ?> post_sticky <?php } ?>">
	...
	<?php if (!is_single()){ ?>
		<header class="entry-header">
			<h2 class="entry-title">
				<a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( esc_html__( 'Permalink to %s', 'tophot' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark"><?php echo get_the_title();  ?></a>
			</h2>
		</header>
	<?php } ?>
	...
	<div class="entry-content">
		...
		?>
		<?php wp_link_pages( array( 'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'tophot' ), 'after' => '</div>' ) ); ?>
	</div><!-- .entry-content -->
	<?php } ?>
